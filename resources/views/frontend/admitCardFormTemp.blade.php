...
<div class="row">
  ...
  <h1 class="page-title" style="margin-top:15px;">Advt_01/2024 for the Post of Junior Hindi Translator and Security Officer - Admit Cards</h1>
...
function searchCandidate(query) {
    ...
    fetch("/search-candidate?query=" + query)
...
<script>
    ...
    if (captcha.length == 0) {
        ...
        return false;
					...
					$.ajax
					({
						url:"{{url('/')}}/Search-Admit-Card-Temp?"+DataUri,
						type:'get',
						dataType:'json',
						success:function(data)
						{
							...
							else
							{
								for(var i=0;i<data.length;i++)
								{
									...
									'<td><a href="/uploads/Admit_Card_Data/' + data[i].application_no + '" target="_blank">Download Admit Card</a></td>'+
