...
<article data-history-node-id="923" class="node node--type-about-stqc node--view-mode-full">
  ...
  <div class="node__content">
    ...
    <div class="text-center mb-4">
        <h2 class="fw-bold">Website Sitemap</h2>
        <p class="text-muted">Browse all pages of the website in a structured layout</p>
    </div>
            ...
            <tr>
                ...
                <td rowspan="{{ $rowspan }}" class="fw-bold bg-light">
                    ...
                    @if($mn->menu_type == 'External')
                        <a href="{{ $mn->link }}" target="_blank" class="text-primary">{{ $mn->menu_name }}</a>
                    @else
                        <a href="{{ url('/en/'.$mn->id.'/'.$mn->menu_name) }}" class="text-primary">{{ $mn->menu_name }}</a>
                    @endif
                            ...
                            @if($childItems->count())
                                <ul class="mb-0" style="padding-left:15px;">
                                    @foreach($childItems as $cmn)
                                        ...
                                        <li><a href="{{ $childUrl }}">{{ $cmn->child_menu }}</a></li>
