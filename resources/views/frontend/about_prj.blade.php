...
<div class="row">
  ...
  <h1 class="page-title">{{$sitecontents->heading}}</h1>
  @endif
  ...
  <div class="node__content">
            ...
            <div class="clearfix text-formatted field field--name-body field--type-text-with-summary field--label-hidden field__item">
               <h1 style="margin-top:20px;">
                 Record Will Be Updated Soon...
              </h1>
            </div>
            @endif
         @if($main_cat == 33)
         @if(!empty($staff_area))
          <h1 class="page-title" style="margin-top:10px;">Scientist</h1>
              ...
              @foreach($staff_area as $st)
                  <div class="col-md-3 col-sm-6 mb-4">
                      <div class="team-member">
                          ...
                          >
                          <h5 class="mt-2 font-weight-bold text-primary">{{ $st->name }}</h5>
                          <a href="{{ url('en/ViewProfile/' . $st->id . '/' . urlencode($st->name)) }}">
                              <p class="text-muted">{{ $st->deg }}</p>
                          </a>
                          <a href="{{ url('en/ViewProfile/' . $st->id . '/' . urlencode($st->name)) }}" class="btn btn-primary" target="_blank">
                              View Profile
                          </a>
                      </div>
                  </div>
      ...
      @endif
     @if(!empty($staff_area))
          <h1 class="page-title" style="margin-top:10px;">Technical Staff</h1>
          <div class="row text-center">
