...
<html lang="en" dir="ltr">
  ...
  <head>
    ...
    <title>NBRI | CSIR-National Botanical Research Institute</title>
          ...
          <div class="row">
            ...
            <div class="col-12 col-sm-12 col-md-12 col-lg-6">
              ...
              <ul style="height: 20px;">
                ...
                <li class="hide"><a href="#mainsection" class="skiptomain">Skip to Main Content</a></li>
...
<a title="" id="yellowButton" class="black com-color">A</a>-->
               ...
               <!-- <li class="dropdown"><a href="#">T<sup>T</sup></a>
                ...
                <li>
                  <a class="topfb_icon" href="{{url('/')}}/en/sitemap"><img alt="sitemap" class="s_map" src="{{url('/')}}/themes/nbri/images/sitemap.png"></a>
                ...
                <li class="nobdr lang">
                  <div class="region region-language-block">
                    <div class="language-switcher-language-url block block-language block-language-blocklanguage-interface" id="block-stqc-languageswitcher" role="navigation">
                      ...
                      <ul class="links">
                        <li class="links">
                        <select title="Select Language" class="blue com-color" id="languageDropdown" style="padding:0px;margin-top:12px;font-size:9px;margin-bottom:0px;height:25px;">
                         <option value="#" selected data-url="{{url('/')}}/en">English</option>
                ...
                <a class="logo-align" href="{{url('/')}}/" style="text-align: center;">
                  ...
                  <div class="brand-text" id="pp1">
                    ...
                    <p style="font-size: 13px;font-weight:bold;">
                      An Institute of National Importance under Council of Scientific & Industrial Research (CSIR),<br> Department of Scientific and Industrial Research, Ministry of Science and Technology, Government of India
                    </p>
                  </div>
                </a>
              </div>
                        ...
                        <div id="block-stqc-mainmenu" class="block block-superfish block-superfishmain-menu">
                          ...
                          <ul id="superfish-main-menu" class="menu sf-menu sf-main-menu sf-horizontal sf-style-none">
                            ...
                            <li class="sf-depth-1 sf-no-children ss_new">
                              <a href="{{ url('/') }}" title="Home page of NBRI Website" class="is-active sf-depth-1">
                            ...
                            @foreach($menu as $mn)
                              @if($mn->havesub == 'Yes')
                                <li class="sf-depth-1 mainme menuparent ss_new">
                                  <a href="#" title="{{ $mn->menu_name }}" class="sf-depth-1 menuparent">{{ $mn->menu_name }}</a>
                                  <ul>
                                    @foreach($submenu as $smn)
                                      @if($mn->id == $smn->cat_name)
                                        @if($smn->havechild == 'Yes')
                                          <li class="sf-depth-2 sf-no-children">
                                            <a href="#" class="sf-depth-2">{{ $smn->sub_name }}</a>
                                            <ul>
                                              @foreach($childmenu as $cmn)
                                                @if($smn->id == $cmn->submenu)
                                                  @if($cmn->child_menu == 'Guest House Booking')
                                                  <li class="sf-depth-3 sf-no-children">
                                                     <a href="/en/Guest-House-Booking" class="sf-depth-3">{{$cmn->child_menu}}</a>
                                                  </li>
                                                  @else
                                                  <li class="sf-depth-3 sf-no-children">
                                                    <a href="{{ url('/en/'.$mn->id.'/'.$smn->id.'/'.$cmn->id.'/'.$cmn->child_menu) }}" class="sf-depth-3">{{ $cmn->child_menu }}</a>
                                                  </li>
                                                  @endif
                                                @endif
                                              @endforeach
                                        ...
                                        @else
                                          ...
                                          @if(in_array($smn->sub_name, ['Scientists', 'Administration', 'Technical and Support Staff']))
                                            @php $url = url('/en/Department-Staff-List/'.$smn->id.'/'.$smn->sub_name); @endphp
                                          @elseif($smn->sub_name == 'Director')
                                            @php $url = url('/en/ViewProfile/2/'.$smn->sub_name); @endphp
                                          @elseif($smn->sub_name == 'CSIR')
                                            @php $url = 'https://www.csir.res.in'; @endphp
                                          @else
                                            @php $url = url('/en/'.$mn->id.'/'.$smn->id.'/'.trim(preg_replace('/[^A-Za-z0-9\-]+/', '-', $smn->sub_name), '-')); @endphp
                                          @endif

                                          <li class="sf-depth-2 sf-no-children">
                                            <a href="{{ $url }}" @if($url == 'https://www.csir.res.in') onclick="return ConfirmLeaveSite(this.href)" @endif class="sf-depth-2" {{ $smn->sub_name == 'CSIR' ? 'target=_blank' : '' }}>{{ $smn->sub_name }}</a>
                                          </li>
                                        @endif
                                      @endif
                                    @endforeach
                              ...
                              @elseif($mn->menu_type == 'External')
                                <li class="sf-depth-1 menuparent ss_new mainme">
                                  <a href="{{ $mn->link }}" target="_blank" class="sf-depth-1 menuparent">{{ $mn->menu_name }}</a>
                                </li>
                              @else
                                <li class="sf-depth-1 menuparent ss_new mainme">
                                  <a href="{{ url('/en/'.$mn->id.'/'.$mn->menu_name) }}" class="sf-depth-1 menuparent">{{ $mn->menu_name }}</a>
                                </li>
                              @endif
                            @endforeach
