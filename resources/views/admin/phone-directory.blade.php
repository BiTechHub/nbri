...
<div class="content-wrapper">
  <div class="container">
    ...
    <div class="form-group">
        <h5>Category <span class="text-danger">*</span></h5>
        <div class="controls">
            <select name="category1" id="category2" class="form-control category2">
            ...
            @error('category1') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
    </div>
</div>
<div class="col-md-3">
    <div class="form-group">
        <h5>श्रेणी <span class="text-danger">*</span></h5>
        <div class="controls">
            <select name="category2" id="category23" class="form-control category2">
                <option value="" selected>--कृपया श्रेणी चुनें--</option>
                @foreach($category_hii as $categorymaster)
                <option value="{{$categorymaster->id}}" {{ (isset($category_id->id) && $category_id->id == $categorymaster->id) ? 'selected' : '' }}>{{$categorymaster->name}}</option>
                @endforeach
            </select>
            @error('category2') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
    </div>
</div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('category23').addEventListener('change', function() {
      var selectedCategoryId = this.value;
      if (selectedCategoryId !== "") {
        var url = "{{url('/')}}/admin-panel/phone-directory/" + selectedCategoryId;
        window.location.href = url; // Redirect to the generated URL
